...
#the following parameters are add to simulate the detritus @Mo
mr = 1E-19/86400 # (s-1) mortality rate of phytoplankton
...
E = E3.E
Qc = 1.00 * 10 ** (
    -12) / 12  # (molC/cell) biomass C per cell (196-18)(average of N and P limited cases from Healey 1985)
YchlN_C = 4 / 55
...
YnucacidP_N = 1 / (3.5 * (
            1 - CG) + 4 * CG)  # (molP molN-1) P/N molar ratio of RNA (193-26) values (193-28) excel file "08 N to P ratio in DNA and RNA.xlsx"

YdnaC_N = 3.5 * (
            1 - CG) + 2.5 * CG  # (molC molN-1) C/N molar ratio of dna (based on "10 Amino acids and nucleic acids stoichiometry.xlsx)
YrnaC_N = 3.25 * (
            1 - CG) + 2.5 * CG  # (molC molN-1) C/N molar ratio of rna (based on "10 Amino acids and nucleic acids stoichiometry.xlsx)
...
for i in Loop_array:
    ...
    B = Nconst_protein + (m * Ynphoto_chl+mr) / Pchl
...
# OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
Chl = ((1 + E) * D * Qc + m + mr) / Pchl  # (molC chl cell-1) cN[i]hlrophyll concentration (193-25)
# =========================
# Nitrogen related
# =========================
...
# Preparation---------------------------------------
# Chlorophyll related
Chl_const = ( m + mr ) / Pchl  # (molC chl cell-1) cN[i]hlrophyll concentration (193-25) Bchl(I)
...
Qc_const = (m + mr) / Pchl + B * CNprotein + M * YpgC_P + Nrna_const * YrnaC_N + Ndna_const * YdnaC_N + Cessential
...
StackPlotColorsC = (
Color_Cessential, Color_protein_const, Color_photo, Color_protein_biosynthesis, Color_DNA, Color_RNA, Color_chl,
Color_thylakoid, \
Color_Nstore, Color_other)
StackPlotColorsN = (
Color_protein_const, Color_photo, Color_protein_biosynthesis, Color_DNA, Color_RNA, Color_chl, Color_Nstore)
...
figure(2)
stackplot(NO3, Nconst_protein_plot + Ndna_const_plot, Nphoto_plot + Nchl_plot, Nbiosynth_plot + Nrna_plot,
          colors=StackPlotColorsN2)
...
stackplot(NO3, Cessential_plot + Cconst_protein_plot + Cdna_const_plot, \
          Cphoto_plot + CthylakoidPG_plot + Cchl_plot, Crna_plot + Cbiosynth_plot, Cnstore_plot, Cother_plot,
          colors=StackPlotColorsC2)
...
# ============================
# Detritus Generation Module
# ============================
def detritus_module(D, protein, lipid, chlorophyll, carbohydrate, rna, dna, death_rate, degradation_rate):
    """
    Simulates the detritus composition after phytoplankton death.

    Parameters:
    D: array-like
        Phytoplankton growth rate (mol cell-1 s-1).
    protein, lipid, chlorophyll, carbohydrate, rna, dna: array-like
        Different macromolecular contents in the phytoplankton.
    death_rate: float
        Death rate of the phytoplankton (1/s).
    degradation_rate: dict
        Degradation rates of different macromolecular contents in detritus (1/s).

    Returns:
    detritus_composition: dict
        Dictionary containing detritus composition over time for each macromolecule type.
    """

    # Initial proportion of each macromolecular component in the detritus based on phytoplankton composition
    detritus_composition = {
        'protein': protein * death_rate,
        'lipid': lipid * death_rate,
        'chlorophyll': chlorophyll * death_rate,
        'carbohydrate': carbohydrate * death_rate,
        'rna': rna * death_rate,
        'dna': dna * death_rate
    }

    # Simulating detritus degradation over time (assuming a constant degradation rate for simplicity)
    for key in detritus_composition:
        # Apply degradation rate to each macromolecular component
        detritus_composition[key] = detritus_composition[key] * (1 - degradation_rate.get(key, 0.0))

    return detritus_composition

# ============================
# Integration in the main code
# ============================

# Define death rate and degradation rate for detritus module
mortality_rate = 1E-19/86400  # Example death rate constant (1/s)
degradation_rate = {
    'protein': 0.015/86400,
    'lipid': 1e-7,
    'chlorophyll': 1e-7,
...
# Calculate detritus composition using the new module
detritus_composition = detritus_module(D, Nprotein, Cessential, Chl, Nchl, Nrna, Ndna, mortality_rate, degradation_rate)

# Store detritus composition in separate variables for clarity
detritus_protein = detritus_composition['protein']
detritus_lipid = detritus_composition['lipid']
detritus_chlorophyll = detritus_composition['chlorophyll']
detritus_carbohydrate = detritus_composition['carbohydrate']
detritus_rna = detritus_composition['rna']
detritus_dna = detritus_composition['dna']

# Calculate the percentage composition of each macromolecule in detritus
total_detritus = detritus_protein + detritus_lipid + detritus_chlorophyll + detritus_carbohydrate + detritus_rna + detritus_dna

# Percentage of each component
protein_percentage = (detritus_protein / total_detritus) * 100
lipid_percentage = (detritus_lipid / total_detritus) * 100
chlorophyll_percentage = (detritus_chlorophyll / total_detritus) * 100
carbohydrate_percentage = (detritus_carbohydrate / total_detritus) * 100
rna_percentage = (detritus_rna / total_detritus) * 100
dna_percentage = (detritus_dna / total_detritus) * 100

# Create a stack plot to show the percentage composition of each component
plt.figure(figsize=(12, 8))
plt.stackplot(NO3, protein_percentage, lipid_percentage, chlorophyll_percentage, carbohydrate_percentage, rna_percentage, dna_percentage,
              labels=['Protein', 'Lipid', 'Chlorophyll', 'Carbohydrate', 'RNA', 'DNA'], alpha=0.8)
plt.xlabel('Nitrate Concentration (NO3) [umol L-1]')
plt.ylabel('Percentage Composition (%)')
plt.title('Detritus Macromolecular Composition vs Nitrate Concentration')
