...
def detritus_dynamics(X, mr, NO3_input, time):
    ...
    population_lipid = detritus_macromolecular["population_lipid"]
    # 假设 Nchl 是你要检查的变量
    if isinstance(population_Nchl, np.ndarray):
        print("population_Nchl is NumPy array")
    else:
        print("population_Nchl is a single value")

    """calculate the detritus production based on the mortality 基于死亡率计算碎屑生成"""
    ...
    Detritus_lipid = np.zeros(len(time))
    ...
    Detritus_lipid[0] = population_lipid[0]
    dt = time[1] - time[0]
    for t in range(1, len(time)):
        protein_degradation = degradation_rate['protein'] * Detritus_protein[t - 1]**2 * dt
        chl_degradation = degradation_rate['chl'] * Detritus_chl[t - 1] ** 2 * dt
        dna_degradation = degradation_rate['dna'] * Detritus_dna[t - 1] ** 2 * dt
        rna_degradation = degradation_rate['rna'] * Detritus_rna[t - 1] ** 2 * dt
        lipid_degradation = degradation_rate['lipid'] * Detritus_lipid[t - 1] ** 2 * dt

        Detritus[t] = Detritus[t-1] + mr * X[t-1] ** 2 * dt
        print(f"Step {t}: Detritus_protein = {Detritus_protein[t - 1]}, X = {X[t - 1]}")

        # 蛋白质碎屑的生成和降解
        Detritus_protein[t] = Detritus_protein[t - 1] + mr ** 2 * Detritus_protein[t - 1] * dt \
...
# visualize the population growth and detritus可视化种群增长和碎屑生成
